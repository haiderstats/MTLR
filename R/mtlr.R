...
mtlr <- function(formula,
  ...
  if(type =="right"){
    ...
    y_cen <- matrix(1 - Reduce(c,Map(function(ind) time[ord] <= c(0,time_points)[ind], seq_along(time_points))), ncol = length(time_points), byrow = T)
    y_uncen <- matrix(1 - Reduce(c,Map(function(ind) time[ord] > time_points[ind], seq_along(time_points))), nrow = length(time_points), byrow = T)
    y_matrix <- cbind(y_cen,y_uncen)
    ...
    y_left <- matrix(as.numeric(Reduce(c,Map(function(ind) time[delta == -2] >= c(0,time_points)[ind],
                                             seq_along(c(time_points,Inf))))), nrow = length(time_points) +1, byrow = T)
    y_right <- matrix(as.numeric(Reduce(c,Map(function(ind) time[delta == 0] < c(time_points,Inf)[ind], seq_along(c(time_points,Inf))))), nrow = length(time_points)+1, byrow = T)
    y_uncen <- matrix(as.numeric(Reduce(c,Map(function(ind) time[delta == 1] < c(time_points,Inf)[ind], seq_along(c(time_points,Inf))))), nrow = length(time_points)+1, byrow = T)
